...
<div class="p-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-semibold text-gray-100">Digital Product Details</h1>
        <div class="space-x-3">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-card rounded-lg shadow-md p-6 border border-gray-800">
            <h2 class="text-lg font-semibold text-gray-100 mb-4">Product Information</h2>
            
            <dl class="space-y-3">
                <div>
                    <dt class="text-sm font-medium text-gray-400">Name</dt>
                    <dd class="text-base text-gray-100">{{ $digitalProduct->name }}</dd>
                </div>
                
                <div>
                    <dt class="text-sm font-medium text-gray-400">Description</dt>
                    <dd class="text-base text-gray-100">{{ $digitalProduct->description ?: 'No description' }}</dd>
                </div>
                
                <div>
                    <dt class="text-sm font-medium text-gray-400">Price</dt>
                    <dd class="text-base text-gray-100">Rs. {{ number_format($digitalProduct->price, 2) }}</dd>
                </div>
                ...
                <div>
                    <dt class="text-sm font-medium text-gray-400">Featured</dt>
                    <dd class="text-base text-gray-100">
        ...
        <!-- PDF Information -->
        <div class="bg-card rounded-lg shadow-md p-6 border border-gray-800">
            <h2 class="text-lg font-semibold text-gray-100 mb-4">PDF Information</h2>
            
            <dl class="space-y-3">
                <div>
                    <dt class="text-sm font-medium text-gray-400">File Status</dt>
                    <dd class="text-base text-gray-100">
                        @if($digitalProduct->pdf_file_path)
                            <span class="text-green-400">PDF Uploaded</span>
                        @else
                            <span class="text-red-400">No PDF Uploaded</span>
                        @endif
                    </dd>
                </div>
                ...
                <div>
                    ...
                    <dd class="text-base text-gray-100">{{ $digitalProduct->formatted_file_size }}</dd>
                </div>
                @endif
                
                @if($digitalProduct->page_count)
                <div>
                    <dt class="text-sm font-medium text-gray-400">Page Count</dt>
                    <dd class="text-base text-gray-100">{{ $digitalProduct->page_count }} pages</dd>
                </div>
                @endif
                
                <div>
                    <dt class="text-sm font-medium text-gray-400">Created At</dt>
                    <dd class="text-base text-gray-100">{{ $digitalProduct->created_at->format('M d, Y h:i A') }}</dd>
                </div>
                
                <div>
                    <dt class="text-sm font-medium text-gray-400">Last Updated</dt>
                    <dd class="text-base text-gray-100">{{ $digitalProduct->updated_at->format('M d, Y h:i A') }}</dd>
                </div>
            </dl>
        </div>
    </div>
    ...
    <!-- Users with Access -->
    <div class="mt-6 bg-card rounded-lg shadow-md border border-gray-800">
        <div class="px-6 py-4 border-b border-gray-800">
            <h2 class="text-lg font-semibold text-gray-100">Users with Access ({{ $users->count() }})</h2>
        </div>
                    ...
                    <tr class="hover:bg-darker/50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-400">
                            @if($user->pivot->order_id)
                                <a href="{{ route('admin.orders.show', $user->pivot->order_id) }}" class="text-indigo-400 hover:text-indigo-300">
                                    View Order
                                </a>
                            @else
                                N/A

...
<div class="glass-effect rounded-lg overflow-hidden shadow-lg border border-gray-800 p-6">
    <div class="flex items-center mb-4">
        ...
        <h3 class="text-xl font-medium text-white">Edit Product: <span class="text-accent-teal">{{ $digitalProduct->name }}</span></h3>
    </div>
    
    <form action="{{ route('admin.digital-products.update', $digitalProduct) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="4" 
                             ...
                             placeholder="Provide details about this digital product...">{{ old('description', $digitalProduct->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                <div class="mb-4">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="0.01" name="price" id="price" value="{{ old('price', $digitalProduct->price) }}" 
                ...
                <div class="mb-4">
                    ...
                    <select name="type" id="type" 
                           ...
                           required>
                        <option value="license_key" {{ old('type', $digitalProduct->type) === 'license_key' ? 'selected' : '' }}>License Key</option>
                        <option value="account_credentials" {{ old('type', $digitalProduct->type) === 'account_credentials' ? 'selected' : '' }}>Account Credentials</option>
                        <option value="digital_asset" {{ old('type', $digitalProduct->type) === 'digital_asset' ? 'selected' : '' }}>Digital Asset</option>
                    </select>
                    @error('type')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror

...
<div class="glass-effect rounded-lg overflow-hidden shadow-lg border border-gray-800 p-6">
    <div class="flex items-center mb-4">
        ...
        <h3 class="text-xl font-medium text-white">Edit Course: <span class="text-accent-teal">{{ $course->title }}</span></h3>
    </div>
    ...
    <form action="{{ route('admin.courses.update', $course) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="5" 
                     ...
                     placeholder="Describe the course content and objectives">{{ old('description', $course->description) }}</textarea>
            @error('description')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="tags[]" id="tags" multiple 
                ...
                @foreach($tags as $id => $name)
                    <option value="{{ $id }}" {{ (old('tags', $selectedTags) && in_array($id, old('tags', $selectedTags))) ? 'selected' : '' }}>{{ $name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <div class="mt-1 flex items-center">
                @if($course->thumbnail)
                    <div class="relative">
                        <img src="{{ asset('storage/' . $course->thumbnail) }}" alt="{{ $course->title }}" class="w-20 h-20 object-cover rounded-md">
                        <span class="absolute top-0 right-0 h-4 w-4 rounded-full bg-accent-teal flex items-center justify-center" title="Current thumbnail">
            ...
            <p class="text-gray-500 text-xs mt-1">Upload a new image to change the current thumbnail</p>
            @error('thumbnail')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
